...
def write_shp_tiles(out_filepath, tiles):
    ...
    layer_out.CreateField(new_field)
    new_field = ogr.FieldDefn('EASTINGLL', ogr.OFTInteger)
    layer_out.CreateField(new_field)
    new_field = ogr.FieldDefn('NORTHINGLL', ogr.OFTInteger)
    layer_out.CreateField(new_field)
    ...
    for tile_counter in range(0, tiles.shape[2]):
        ...
        feat.SetField('COVERSLAND', 1)
        extent = int(np.max(points[:, 0]) - np.min(points[:, 0]))
        feat.SetField('EXTENT', extent)
        feat.SetField('EASTINGLL', int(np.min(points[:, 0])))
        feat.SetField('NORTHINGLL', int(np.min(points[:, 1])))
...
def main():
    dirname = r"D:\work\data\LatLonGrid\tiles" #r"D:\work\code\latlon_grid\LatLonGrid\latlongrid\grids"
    version = "V1"
    sub_gridname = "GL"
    tile_size = 6
    bbox = [-180, -90, 180, 90]
